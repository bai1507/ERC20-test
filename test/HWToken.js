...
describe("Token contract", function () {
  ...
  before(async function () {
    ...
    Factory = await ethers.getContractFactory("OutToken", { from: owner });

    // Token名称：HWToken
    // Token符号: HWT
    // Token精度: 18
    // Token总量: 10000枚
    hardhatToken = await Factory.deploy(INITIAL_SUPPLY);
  ...
  describe("Deployment", function () {
    ...
    it("check transferFrom", async () => {
      ...
      await hardhatToken
        .connect(addr2)
        .transferFrom(owner.address, addr2.address, 50);
      expect(
        await hardhatToken.allowance(owner.address, addr2.address)
      ).to.equal(450);
    });
    ...
    it("can not transfer above the amount", async () => {
      ...
      await expect(hardhatToken.connect(addr1).transfer(addr3.address, 1007)).to
        .be.reverted;
    });
  });
});
